...
const { makeJsonFromQBXML, saveJSONFile } = require("./utils");
...
var qbws,
  ...
  companyFile =
    process.env.QB_COMPANY_FILE_PATH ||
 ...
 *   BillQuery
 */
// Use GeneralDetailReportType for existing types of reports
...
// Use CustomDetailReportType for when you need control of everything
...
function buildRequest() {
  ...
  // This example CustomDetailReportQueryRq is working
  // from https://github.com/IntuitDeveloper/QBXML_SDK_Samples/blob/64bitUpgrade/xmlfiles/legacy/CustomDetailReport.xml
  inputXMLDoc = builder
    .create("QBXML", { version: "1.0" })
    .instruction("qbxml", 'version="4.0"')
    .ele("QBXMLMsgsRq", { onError: "stopOnError" })
    .ele("CustomDetailReportQueryRq", { requestID: "UUIDTYPE" })
    .ele("CustomDetailReportType", "CustomTxnDetail")
    .up()
    // // .ele("ReportDateMacro", "ThisMonth")
    // // .up()
    .ele("ReportPeriod")
    .ele("FromReportDate", "2025-05-01")
    .up()
    .ele("ToReportDate", "2025-05-31")
    .up()
    .up()
    .ele("SummarizeRowsBy", "Quarter")
    .up()
    .ele("IncludeColumn", "TxnNumber")
    .up()
    .ele("IncludeColumn", "TxnType")
    .up()
    .ele("IncludeColumn", "Date")
    .up()
    .ele("IncludeColumn", "Name")
    .up()
    .ele("IncludeColumn", "Memo")
    .up()
    .ele("IncludeColumn", "Account")
    .up()
    .ele("IncludeColumn", "Class")
    .up()
    .ele("IncludeColumn", "SplitAccount")
    .up()
    .ele("IncludeColumn", "Debit")
    .up()
    .ele("IncludeColumn", "Credit")
    .up()
    .ele("IncludeColumn", "Amount")
    .up()
    .up()
    .up();
  ...
  // This Query to the GL Report with all defaults is working
  // inputXMLDoc = builder
  //   .create("QBXML", { version: "1.0" })
  //   .instruction("qbxml", 'version="4.0"')
  //   .ele("QBXMLMsgsRq", { onError: "stopOnError" })
  //   .ele("GeneralDetailReportQueryRq")
  //   .ele("GeneralDetailReportType")
  //   .text("GeneralLedger");
  // strRequestXML = inputXMLDoc.end({ pretty: false });
  // request.push(strRequestXML);

  // Examples that came with the repo
  /*{
    // CustomerQuery
    inputXMLDoc = builder
        .create("QBXML", { version: "1.0" })
...
function announceMethod(name, params) {
  ...
  if (config.verbosity > 0) {
    serviceLog(
      chalk.green("WebMethod: " + name + "() has been called by QBWebConnector")
    );
  }
      ...
      for (arg in params) {
        if (params.hasOwnProperty(arg)) {
          ...
          if (argType === "object") {
            ...
            serviceLog("        " + argType + " " + arg + " = " + params[arg]);
          }
        }
      }
    } else {
...
qbws.QBWebConnectorSvc.QBWebConnectorSvcSoap.authenticate = function (args) {
  ...
  if (
    args.strUserName.trim() === username &&
    args.strPassword.trim() === password
  ) {
    req = buildRequest();
 ...
 */
qbws.QBWebConnectorSvc.QBWebConnectorSvcSoap.getInteractiveURL = function (
  args
) {
  var retVal = "";
 ...
 */
qbws.QBWebConnectorSvc.QBWebConnectorSvcSoap.interactiveRejected = function (
  args
) {
  var retVal = "";
 ...
 */
qbws.QBWebConnectorSvc.QBWebConnectorSvcSoap.receiveResponseXML = function (
  args
) {
  ...
  // which message type do we care about
  const msgType = "CustomDetailReportQueryRs";
  let msgVal = null;

  const outputJson = convert(response, { format: "json" });
  const printObj = JSON.parse(outputJson);
  msgVal = printObj?.QBXML?.QBXMLMsgsRs?.[msgType];

  if (msgVal) {
    console.log("---------------------------------------------------------");
    // Prints { QBXML: { QBXMLMsgsRs: etc
    //console.log("in receiveResponseXML, this is printObj", printObj);
    console.log("in receiveResponseXML,", { msgType }, "msgVal:", msgVal);
    // this has column metadata possibly needed?
    // console.log("ReportRet:", msgVal?.ReportRet);

    const parseMsg = makeJsonFromQBXML(msgVal);

    ////console.log("ReportData.ColDesc:", msgVal?.ReportRet?.ColDesc);

    // console.log("ReportData:", msgVal?.ReportRet?.ReportData);

    // Doesn't look like we need the total row for our purposes
    // console.log(
    //   "ReportData['TotalRow']:",
    //   msgVal?.ReportRet?.ReportData?.TotalRow
    // );

    // Lots of rows of data
    // console.log(
    //   "ReportData['DataRow']:",
    //   msgVal?.ReportRet?.ReportData?.DataRow
    // );

    // // First row of data looks like:
    // console.log(
    //   "1st ReportData['DataRow']:",
    //   msgVal?.ReportRet?.ReportData?.DataRow?.[0]
    // );

    saveJSONFile(parseMsg, "./" + new Date().getTime() + "-QBXML.json");
    console.log("---------------------------------------------------------");
  }
...
module.exports.run = function runQBWS() {
  ...
  console.log('server listening at port 8000, host 0.0.0.0')
